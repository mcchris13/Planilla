	...
	if(isset($_POST['employee'])){
		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					$output['error'] = true;
					$output['message'] = 'Ya registraste tu entrada por hoy';
				}
				else{
					//updates
					...
					if($conn->query($sql)){
						$output['message'] = 'Empleado: '.$row['firstname'].' '.$row['lastname'];
					}
					else{
						$output['error'] = true;
				...
				if($query->num_rows < 1){
					$output['error'] = true;
					$output['message'] = 'Primero registra tu entrada';
				}
				else{
					$row = $query->fetch_assoc();
					if($row['time_out'] != '00:00:00'){
						$output['error'] = true;
						$output['message'] = 'Ya registraste tu salida por hoy';
					}
					else{
